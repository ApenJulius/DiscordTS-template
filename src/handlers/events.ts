...
module.exports = (client: Client) => {
    ...
    readdirSync(eventsDir).forEach(file => {
        ...
        else if (event.type === EventType.REST_ON) //TODO: Make this work with rate limit
            // eslint-disable-next-line
            // @ts-ignore
            client.rest.on(event.name, consola.log)
        else
            consola.error(`${event.name} doesn't have a correct type from the enum 'EventType, but has ${event.type}`)
